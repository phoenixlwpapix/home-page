// src/data/projects.js
export const projects = [
  {
    title: "知了英语",
    description: "AI驱动的英语学习平台",
    imageUrl:
      "https://placehold.co/600x400/33A9D8/FFFFFF?text=Cicada%20English",
    demoUrl: "https://cicada-english.studioyyh.tech/",
    githubUrl: "https://github.com/phoenixlwpapix/cicada-english",
  },
  {
    title: "数理化工具箱",
    description: "帮你更好理解数学、物理、化学知识。",
    imageUrl: "https://placehold.co/600x400/71A9F7/FFFFFF?text=Math%20Toolkit",
    demoUrl: "https://math-toolkit.studioyyh.tech/",
    githubUrl: "https://github.com/phoenixlwpapix/math-toolkit",
  },
  {
    title: "PRD精灵",
    description: "帮你更好编写项目PRD文件并建议技术栈。",
    imageUrl: "https://placehold.co/600x400/FD6F60/FFFFFF?text=PRD%20Genie",
    demoUrl: "https://prd-genie.studioyyh.tech/",
    githubUrl: "https://github.com/phoenixlwpapix/prd-genie",
  },
  {
    title: "Chez Liliane",
    description: "一个烘焙爱好者的个人主页。",
    imageUrl: "https://placehold.co/600x400/ffc0cb/FFFFFF?text=Chez%20Liliane",
    demoUrl: "https://bakery.studioyyh.tech/",
    githubUrl: "https://github.com/phoenixlwpapix/phoenixlwpapix.github.io",
  },
  {
    title: "Cosmic Flow",
    description:
      "一个动态占星与月相追踪器，提供实时宇宙能量报告、个性化能量指引和行星影响的运用练习。",
    imageUrl: "https://placehold.co/600x400/96E6B3/FFFFFF?text=Cosmic%20Flow",
    demoUrl: "https://cosmic-flow.studioyyh.tech/",
    githubUrl: "https://github.com/phoenixlwpapix/cosmic-flow",
  },
  {
    title: "Weather Wizard",
    description: "A magical weather forecasting app with AI predictions.",
    imageUrl:
      "https://placehold.co/600x400/87CEEB/FFFFFF?text=Weather%20Wizard",
    demoUrl: "https://weather-wizard.studioyyh.tech/",
    githubUrl: "https://github.com/phoenixlwpapix/weather-wizard",
  },
  {
    title: "Recipe Rover",
    description:
      "Explore recipes from around the world with smart ingredient matching.",
    imageUrl: "https://placehold.co/600x400/FFA500/FFFFFF?text=Recipe%20Rover",
    demoUrl: "https://recipe-rover.studioyyh.tech/",
    githubUrl: "https://github.com/phoenixlwpapix/recipe-rover",
  },
];
